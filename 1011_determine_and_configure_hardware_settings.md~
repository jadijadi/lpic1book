...
There are a lot of devices in /dev/ and if you plugin any device, it will have a file in /dev (say /dev/sdb2). **udev** lets you control what will be what in /dev. For example you can use a rule to force your 8GB flash drive with one specifiv vendar to be /dev/mybackup all the time or you can tell it to copy all photos to your home directory as soon as your camera is connected.
...
1000000
````
Another very useful directory here, is `/proc/sys/net/ipv4` which controls realtime networking configurations. 
...
but nobody uses `insmod` because it does not understands dependencies. Instad use the `modprobe` command:
...
If you need to load some modules eveythime your system boots do one of the follow:
