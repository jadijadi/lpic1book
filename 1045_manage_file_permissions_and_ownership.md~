...
It is possilbe to change the permissions on files & directories using the `chmod` command. There are to ways to tell this command what you want to do:
...
> you can tell `chmod` whos permission shoulb be granted or removed by doing things like `u+r` (give read to user), `og-w` (remove write for other and group). 
...
## Access modes
So you have access only to your files. But how you should change your password? or use programs which needs access to system files? You shoud be able to access /etc/passwd or /etc/shadow to change your password but you should not be able to access other people files!
...
Did you note the `s` in the palce of *executable bit* for the user and for the group? That means when any user runs this program, it will be run be the access of the owner of the file (which is root) instead of that users id. 
